...
# death BEFORE stroke within interval 1 (death_t <= stroke_t): stroke unobserved
...
logline(sprintf("person-period rows: %d | events: %d | subjects: %d",
                nrow(pp), sum(pp$event), n_distinct(pp$ID_12)))

ppd <- svydesign(ids = ~communityID, strata = ~urban_nbs, weights = ~w,
                 data = pp, nest = TRUE)

fit_dt <- function(form, tag) {
  m <- svyglm(form, family = quasibinomial(), design = ppd)
  s <- coef(summary(m))
